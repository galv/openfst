...
MappedFile::~MappedFile() {
  if (region_.size != 0) {
    if (region_.mmap) {
      VFST_LOG(1) << "munmap'ed " << region_.size << " bytes at " << region_.mmap;
      if (munmap(region_.mmap, region_.size) != 0) {
        FST_LOG(ERROR) << "Failed to unmap region: " << strerror(errno);
      }
    } else {
      if (region_.data) {
...
MappedFile *MappedFile::Map(std::istream *istrm, bool memorymap,
  ...
  VFST_LOG(1) << "memorymap: " << (memorymap ? "true" : "false") << " source: \""
          << source << "\""
  ...
  if (memorymap && spos >= 0 && spos % kArchAlignment == 0) {
    ...
    if (fd != -1) {
      ...
      if (close(fd) == 0 && map != MAP_FAILED) {
        ...
        if (istrm) {
          VFST_LOG(1) << "mmap'ed region of " << size << " at offset " << pos
                  << " from " << source << " to addr " << map;
          return mmf.release();
        }
      } else {
        FST_LOG(INFO) << "Mapping of file failed: " << strerror(errno);
      }
    }
  }
  // If all else fails, reads from the file into the allocated buffer.
  if (memorymap) {
    FST_LOG(WARNING) << "File mapping at offset " << spos << " of file " << source
  ...
  while (size > 0) {
    ...
    if (!istrm->read(buffer, next_size)) {
      FST_LOG(ERROR) << "Failed to read " << next_size << " bytes at offset "
                 << current_pos << "from \"" << source << "\"";
      return nullptr;
    }
    size -= next_size;
    buffer += next_size;
    VFST_LOG(2) << "Read " << next_size << " bytes. " << size << " remaining";
  }
  return mf.release();
}
