...
bool Verify(const Fst<Arc> &fst, bool allow_negative_labels = false) {
  ...
  if (start == kNoStateId && ns > 0) {
    FST_LOG(ERROR) << "Verify: FST start state ID not set";
    return false;
  } else if (start >= ns) {
    FST_LOG(ERROR) << "Verify: FST start state ID exceeds number of states";
    return false;
  }
  for (StateIterator<Fst<Arc>> siter(fst); !siter.Done(); siter.Next()) {
    ...
    for (ArcIterator<Fst<Arc>> aiter(fst, state); !aiter.Done(); aiter.Next()) {
      ...
      if (!allow_negative_labels && arc.ilabel < 0) {
        FST_LOG(ERROR) << "Verify: FST input label ID of arc at position " << na
                   << " of state " << state << " is negative";
        return false;
      } else if (isyms && isyms->Find(arc.ilabel) == "") {
        FST_LOG(ERROR) << "Verify: FST input label ID " << arc.ilabel
        ...
        FST_LOG(ERROR) << "Verify: FST output label ID of arc at position " << na
                   << " of state " << state << " is negative";
        return false;
      } else if (osyms && osyms->Find(arc.olabel) == "") {
        FST_LOG(ERROR) << "Verify: FST output label ID " << arc.olabel
                   << " of arc at position " << na << " of state " << state
                   << " is missing from output symbol table \"" << osyms->Name()
                   << "\"";
        return false;
      } else if (!arc.weight.Member()) {
        FST_LOG(ERROR) << "Verify: FST weight of arc at position " << na
                   << " of state " << state << " is invalid";
        return false;
      } else if (arc.nextstate < 0) {
        FST_LOG(ERROR) << "Verify: FST destination state ID of arc at position "
    ...
    if (!fst.Final(state).Member()) {
      FST_LOG(ERROR) << "Verify: FST final weight of state " << state
                 << " is invalid";
      return false;
    }
  }
  ...
  if (fst_props & kError) {
    FST_LOG(ERROR) << "Verify: FST error property is set";
    return false;
  }
  uint64 known_props;
  uint64 test_props =
  ...
  if (!CompatProperties(fst_props, test_props)) {
    FST_LOG(ERROR) << "Verify: Stored FST properties incorrect "
